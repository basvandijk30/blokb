...
def floor(real: float) -> int:
    ...
    return int(real // 1)  # int rondt af richting de 0, dus floor division is nodig om te laten werken met negatieve integers
...
def add_frac(n1: int, d1: int, n2: int, d2: int) -> tuple[int]:
    ...
    Examples:
        ...
        (13, 6)
    """
    # als noemers al hetzelfde zijn is de rest van het algoritme niet nodig
    if d1 == d2:
        return (n1 + n2, d1)
    # kleinste gemene veelvoud om de noemers gelijk te maken
    d = lcm(d1, d2)
    # vermenigvuldig tellers met dezelfde factor als hun noemers, en tel ze op
    n = (d // d1) * n1 + (d // d2) * n2
